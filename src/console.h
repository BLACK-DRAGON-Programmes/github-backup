 ...
 * foreground mode (Section 8b of spec). On Windows 10+, ANSI escape
 * codes are enabled via SetConsoleMode with ENABLE_VIRTUAL_TERMINAL_PROCESSING.
 * In background mode (FreeConsole), console output is suppressed.
 *
 * This module also provides the log viewer mode (Section 10c):
 * when a second instance detects the mutex, it tails the log file
 * with colored output.
 */
 ...
 * Must be called once at startup before any console output.
 * Safe to call even if there's no console (background mode) — it just fails silently.
 */
 ...
 * with ANSI color formatting. Blocks until Ctrl+C.
 *
 * @param log_path  Full path to backup.log
 * @return Never returns normally — exits on Ctrl+C.
 */
